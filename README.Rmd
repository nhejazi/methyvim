...
estimable target parameters that arise in causal inference.
...
1. The _average treatment effect_ (ATE) for discrete exposures/treatments,
2. A _nonparametric variable importance measure_ (NPVI) for continuous exposures
...
multiple testing correction uniquely suited to such settings. Rather generally,
the Benjamini & Hochberg procedure for controlling the False Discovery Rate
(FDR) [@benjamini1995controlling] is applied due to multiple testing issues
inherent to the estimation procedure. Specifically, we apply a modified marginal

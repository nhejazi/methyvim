---
title: '`methyvim`: Targeted, Robust, and Model-free Analysis of Differential
  Methylation in R'
author:
  - name: Nima S. Hejazi
    affiliation:
...
keywords: DNA methylation, differential methylation, epigenetics, causal
  inference, statistical variable importance, machine learning, targeted minimum
...
observing $n$ iid copies of $O$ (i.e., $O_1, \ldots, O_n$), where $O \sim P \in
\mathcal{M}$, which is to say that the random variable $O$ is governed by an
...
through the _true_ (and unknown) probability distribution $P$. As a motivating
...
been the subject of much attention in statistical causal inference **CITATION
...
estimation, it is possible the a given CpG site $j$ may have _too many_
...
proposed algorithm as flexible for the user [@kleinberg2003impossibility].

## Targeted Minimum Loss-Based Estimation and Inference
...
[`tmle`](https://CRAN.R-project.org/package=tmle) R package [@gruber2011tmle].
Generally speaking, a TML estimator is constructed from a few simple components:
(1) an estimator of the propensity score [@rosenbaum1983central] (often denoted
by $g(A, W)$), (2) an estimator of the outcome regression (often denoted $Q(A,
W)$), and (3) a targeting step applied to the initial estimator constructed from
the aforementioned components [@gruber2009targeted]. In the sequel, we describe
...
@vdl2011targeted and @vdl2018targeted; moreover, as a matter of practicality, we
make use of the [`tmle`](https://CRAN.R-project.org/package=tmle)  R package of
@gruber2011tmle as a backend for computing estimators of relevant target
...
algorithm [@wolpert1992stacked, breiman1996stacked, vdl2007super], which ensures
that the resulting ensemble learner satisifies important optimality properties
with respect to cross-validation [@vdl2004asymptotic, @dudoit2005asymptotics,
@vdl2003unified]. Importantly, TML estimators are well suited for inference,
having an asymptotically normal limiting distribution
[@tsiatis2007semiparametric, @vdl2006targeted] $\sqrt{n}(\psi_{n, j} -
\Psi_j(P)) \sim N(0, \sigma_{\text{EIF}}^2)$, where $\sigma_{\text{EIF}}^2$ is
...
nonparametric statistical model_. __The statistical algorithm consists in the
several major steps elaborated upon above and summarized below__:

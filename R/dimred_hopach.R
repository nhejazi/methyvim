...
#'
cluster_w <- function(W, newX = NULL, max_vars, verbose = FALSE) {


  # Identify constant columns in W
  mask_const_var <- apply(W, 2, var)
  names(mask_const_var) <- NULL

  # Remove constant columns
  w_filtered <- W %>%
    as.data.frame(.) %>%
    dplyr::select(which(mask_const_var != 0))
